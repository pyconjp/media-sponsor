...
発表者紹介
Masaru Terui氏は
awsコンサルティングをしています。車内ではserverless担当ということで、車内で使うシステムなどをserverlessで作ってます。
クラウドの技術支援やコンサルティングをしている
インフラと開発の間をふらふらしている
両方の濃い持ちがわかると思っている
r舞うだのデプロイツールを作ったり
...
概要
昨今、注目を集めるServerlessについて、現状最もPythonの利用が盛んであるAWS Lambdaと関連サービスを例として、その考え方やFaaSの動作原理に触れながら、それらの特性を踏まえた設計・実装方法をご紹介していただきました。
非常に多くの情報をなので、本レポート内ですべてをご紹介することができず残念ですが、講演の内容から一部抜粋してご紹介します。
詳細はレポート末尾の参考らんに資料と動画を載せているのでそちらをご確認ください。




![](./36749283010_34e27ba550_z)

まずは、サーバレスがどういうものかというところから、メリットの紹介
一般的な説明ではサーバーレスの使い道が見えづらい
サーバーレスにすることでなにがあるか」を理解することが重要では

あるもの
saasとサーバーレス
functional saas
  functional saas を faasでつないで実装するのがserverlesの文脈s
  特徴
関数がサービス化されることで、いきなり関数から始まることでプロトコルまで隠蔽することがd切る

関数の単位で同時実行数を調整することが可能
期待
　引数に対して返り血
 input/outputに注力できる
 依存関係を気にしない右並
 ...
 コードの紹介
htttpのリクエストをふんcいとんい渡すためのサービス
これを使うと、

functional saas
普通のsaalとの違いは？単独ではサーブソツィテ完結しないで特定の機能を提供するsaasのこと
...
fwの仕組みを知っていた方がいいことがある
同じようにserverlessの仕組みを知ればserverlessを使いこなせるようになるはず

仕組み理論的なところ

function as a serviceの仕組み
実態：コンテナ　バックエンドはdockerが多い
意識すること：コンテナの軌道はvmよりはオーバーヘッドは小さいがある
稼働するythonプロセスのオーバーヘッドがある
再利用することがある
...
faasとは
あらゆる操作をイベントとして扱い
pull型のpub/subモデルでオンデマンドにコンテナ内で実行される関数のこと
と言えるのでは

pull型のpub/subモデルでオンデマンドにコンテナ内で実行される関数のこと

serverlesnsに向いているものは？
iot事例
　計測データをhttpsでうけて、ダイナモというデータストアに入れて、必要な谷に集約してsqsに入れt外部も計算用サーバが計算して外部のサービスにrん系する
 iotと非常に相性がいい。iotじゃなくても、データのクレンジング、inとoutが明確なシステムいている。処理していく各ステップで軌道数を調整することができる
 ...
 　普通のwebapiアプリケーション
  gatewayで受けて
  なんで
  リクエストとレスポンスがいったい父で同期的だから
  アーキテクチャ的にマッチしないだけで、運用面で考えればメリットはある
  
  10畳のピント
  twelvefacterapp
  paasというサービスもコンテナで軌道される　頻繁に軌道停止
  アプリケーションはステートレスにしようなど
  基本的な開発マニフェスト
  この中の半分くらいは強制される
 ...
  イベントはat least once
  べき統制のある処理が必要
  正常系を何回実行しても同じ状態になるようにしないといけない　というものではない
  　どのイベントを適応したかを覚えておいて、二回目は無視するというのもべき等
  
  その他ポイント
  ramudawoローカルで動かすライブラリもあるが個人的には普通にユニットテストを書いた方が安定性、
  効率性もいいと思っている
 ...
  ポイントからいくつか紹介
  ...
  　普通のmwと同じように考えると良い
   サポートや、apiは充実しているか
   設定情報をapiでぬけて、同じように設定できるかは重要
   
   氏の豊富な構築経験に基づくわかりやすい説明と、実装におけるポイントの解説など、非常に実践的な公演
