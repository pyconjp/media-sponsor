...
今のPythonを巡る現状は、OSSの成熟が進むことでさらに教育機関で使われることが多くなり、より成熟したOSSへと育っていくといったサイクルができているということでした。

また、科学計算用のパッケージとして有名な「NumPy, SciPy, pandas, scikit-learn, matplotlib, jupyter」などの他にも、タスクに応じたパッケージが数多くあります。
Pythonのパッケージはチームで開発されている物が多いので、Pythonのエコシステムを生かした開発体制ができている点も特徴として挙げられていました。

さらには、国内外で様々な科学計算系のコミュニティ・イベントが活発化してきています。
PYCON JPをはじめ、PyData, SciPyが有名です。
データ系イベントの本家とも言えるSciPyは今年は９日間も開催しました。
データ系という分野だけでこの期間を実施できるということからも年々科学計算系が盛り上がってきていることを示しています。

## pandsでのOSS活動事例の紹介
pandasエコシステム
単体のパッケージ以外にオプショナルなパッケージが存在している

堀越氏の具体的な活動経緯こみったってどんなことをしている？
活動経緯
pandasのコミッタになるには、質・量の貢献度と、活動期間（１年以上）の実績をもって
コアチームメンバの推薦・投票で決定となります。
こみったに選出されてからは、日々こみったの業務をこなしていきます。例えば、issueへの回答であるとか、コードレビュー 　例えば、プロジェクトのスコープを決めるとか、
  
コミッタとして重要なことは、プロダクト・コミュニティの品質を維持するための仕組みをつくることです。
ユーザビリティを担保しつつ、ツールとして機能の品質も維持しないといけない
OSSですから、オープンでないといけない。興味を持った人が参加しやすい環境というのが重要です。
参加しやすくするために、Github, Gitter, Code Of Conduct, Github Wikiは重要です。

## OSS活動　最初の一歩
そして、講演の最後では、OSS活動をして
OSS活動は、コードの修正だけではありません。宣伝活動や、ノウハウの共有などもその一つです。
まずは、プルリクエストを送ってみましょう。

ハードルに感じる要因
英語やGitの使い方、そもそものPythonの技術力など、それらを気にしすぎることはありません。
何か問題があれば教えてくれます。
