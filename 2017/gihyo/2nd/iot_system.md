...
北神さんは高校時代からハードウェアに手を出し、現在はハードウェアとソフトウェアの両方に精通されている方です。  
最近は[Intel Edisonマスターブック](http://gihyo.jp/book/2017/978-4-7741-8921-5)という本を出版されました。  

北神さんがIoTに精通しているということで、おそらく会場の皆さんは「〜ができました」系のトークを期待されていたと思います。  
...
そうなると、IoTには低レイヤーのC言語がベストなのではという疑問が当然出てきます。  
それに対して北神さんは、「そんなC言語をラップできるPythonこそ最高」と答えます。  
つまり、低レイヤーにアクセスできるC言語に加えて、多様なライブラリを要するPythonが合わさることで、様々なことが実現できるのです。  
例えば温湿度センサーの機能を使ってデータを取得し、サーバでデータを表示、データをDBに格納、そしてネットワークにも繋げるということも可能です。  
...
軽快なトークで会場を沸かせた北神さん。  
公演の時間だけでは足りず、公演後も場所を変えて参加者とIoT話に花を咲かせていたようです。  
